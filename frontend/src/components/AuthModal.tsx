/**
 * AuthModal — Login / Register / Google / Name-only
 * © Ken Baserecha — Boutyflameet
 *
 * Fixes:
 * 1. go() now shows feedback when onSignIn/onRegister returns false
 * 2. Google button shows loading spinner while waiting
 * 3. Modal closes automatically when auth context user is set
 *    (parent LobbyPage watches appUser and calls setShowAuth(false))
 * 4. onKeyDown Enter submits the form
 * 5. Error cleared on mode switch
 */
              ...
              {resetSent ? (
                <div className="text-center py-6">
                  ...
                  <p className="text-white/50 text-sm">
                    Reset link sent to <span className="text-flame-400">{email}</span>
                  </p>
                  <button
                    onClick={() => switchMode('login')}
                    className="mt-5 text-flame-400 text-sm font-bold"
                  >
                    ← Back to sign in
                  </button>
                </div>
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ref={mode !== 'register' ? inputRef : undefined}
                      value={email}
                      onChange={e => setEmail(e.target.value)}
                      type="email"
                      placeholder="Email address"
                      autoComplete="email"
                      onKeyDown={mode === 'reset' ? handleKeyDown : undefined}
                      className={inputWithIconCls}
                    />
                  </div>
                  ...
                  {mode !== 'reset' && (
                    <>
                      ...
                      <button
                        ...
                        disabled={loading || googleLoading}
                        className="w-full py-3 rounded-xl font-semibold text-sm bg-white/[0.06] hover:bg-white/[0.12] border border-white/10 text-white flex items-center justify-center gap-2.5 transition-all disabled:opacity-60"
                      >
                        {googleLoading
                    ...
                    {mode === 'login' ? (
                      <>
                        ...
                        <button
                          ...
                          className="text-flame-400 font-bold hover:text-flame-300 transition-colors"
                        >
                          Create account
                        </button>
                      </>
